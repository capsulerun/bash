...
describe('touch command', () => {
    ...
    it('should return error if parent directory does not exist', async () => {
        ...
        const ctx = createMockContext(['bad/file.txt'], {}, {
            resolvePath: resolvePathMock
        });
    ...
    it('should create multiple files correctly', async () => {
        ...
        expect(result.exitCode).toBe(0);
        expect(executeCodeMock).toHaveBeenCalledWith(expect.anything(), expect.stringContaining("writeFileSync('/workspace/file1.txt'"));
        expect(executeCodeMock).toHaveBeenCalledWith(expect.anything(), expect.stringContaining("writeFileSync('/workspace/file2.txt'"));
    });
});
