...
export const handler: CommandHandler = async ({ state, opts, runtime }: CommandContext) => {
    ...
    let isSourceFolder =  sourceAbsolutePath ? await runtime.executeCode(state, `require('fs').statSync('${sourceAbsolutePath}').isDirectory();`) as boolean : false;
    let isDestinationFolder =  destinationAbsolutePath ? await runtime.executeCode(state, `require('fs').statSync('${destinationAbsolutePath}').isDirectory();`) as boolean : false;

    if(!sourceAbsolutePath) {
        return { stdout: '', stderr: `bash: cp: ${source}: No such file or directory`, exitCode: 1 };
    }
    ...
    return { stdout: '', stderr: `bash: cp: ${destination}: No such file or directory`, exitCode: 1 };
};
