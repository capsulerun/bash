...
describe('ls command', () => {
    it('should list files excluding hidden ones by default', async () => {
        ...
        const ctx = createMockContext(['dir'], {}, { resolvePath: resolvePathMock, executeCode: executeCodeMock });
    ...
    it('should list hidden files with -a', async () => {
        ...
        const ctx = createMockContext(['-a', 'dir'], {}, { resolvePath: resolvePathMock, executeCode: executeCodeMock });
        const result = await handler(ctx);
    ...
    it('should use long format with -l', async () => {
        ...
        const executeCodeMock = vi.fn().mockImplementation(async (state, code) => {
            ...
            if (code.includes('statSync')) return { mode: 0o100644, size: 1024, nlink: 1, mtime: '2023-01-01T00:00:00.000Z' };
            return {};
        });

        const ctx = createMockContext(['-l', 'dir'], {}, { resolvePath: resolvePathMock, executeCode: executeCodeMock });
        const result = await handler(ctx);
    ...
    it('should support multiple directories', async () => {
        ...
        const ctx = createMockContext(['dir1', 'dir2'], {}, { resolvePath: resolvePathMock, executeCode: executeCodeMock });
        const result = await handler(ctx);
