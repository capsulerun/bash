...
describe('find command', () => {
    ...
    it('should filter by -type f', async () => {
        ...
        const executeCodeMock = vi.fn().mockImplementation(async (state, code) => {
            if (code.includes('statSync(\'/workspace/dir\')')) return { isDirectory: true, entries: ['f1.txt'] };
            return { isDirectory: false };
        });

        const ctx = createMockContext(['dir', '-type', 'f'], {}, {
            resolvePath: resolvePathMock,
            executeCode: executeCodeMock
        });
    ...
    it('should filter by -name pattern', async () => {
        ...
        const executeCodeMock = vi.fn().mockImplementation(async (state, code) => {
            if (code.includes('statSync(\'/workspace/dir\')')) return { isDirectory: true, entries: ['match.ts', 'ignore.js'] };
            return { isDirectory: false };
        });

        const ctx = createMockContext(['dir', '-name', '*.ts'], {}, {
            resolvePath: resolvePathMock,
            executeCode: executeCodeMock
        });
