...
function parseRawArgs(raw: string[]): { searchPath: string; name: string | null; type: 'f' | 'd' | null } {
    let searchPath = '.';
...
export const handler: CommandHandler = async ({ opts, state, runtime }: CommandContext) => {
    ...
    if (!absolutePath) {
        return { stdout: '', stderr: `bash: find: '${searchPath}': No such file or directory`, exitCode: 1 };
    }
    ...
    const walk = async (absPath: string, displayPath: string) => {
        ...
        if (info.isDirectory) {
            await Promise.all((info.entries ?? []).map(entry =>
                walk(`${absPath}/${entry}`, `${displayPath}/${entry}`)
            ));
        }
    };
