...
describe('curl command', () => {
    ...
    it('should parse headers, method and body correctly', async () => {
        ...
        const ctx = createMockContext(['-X', 'POST', '-H', 'Content-Type: application/json', '-d', '{"a":1}', 'https://api.example.com'], {}, { executeCode: executeCodeMock });
        ctx.opts.raw = ['-X', 'POST', '-H', 'Content-Type: application/json', '-d', '{"a":1}', 'https://api.example.com'];
    ...
    it('should not output error with silent flag', async () => {
        ...
        const ctx = createMockContext(['-s', 'https://example.com'], {}, { executeCode: executeCodeMock });
