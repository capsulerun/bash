...
export const handler: CommandHandler = async ({ opts, state, runtime }: CommandContext) => {
    ...
    if (inline) {
        ...
        if (!code) {
            return { stdout: '', stderr: 'bash: python3: -c requires a code argument', exitCode: 1 };
        }

        if (rest.length > 0) {
            return { stdout: '', stderr: 'bash: python3: unexpected arguments after -c', exitCode: 1 };
        }
    ...
    if (!absolutePath) {
        return { stdout: '', stderr: `bash: python3: ${file}: No such file or directory`, exitCode: 1 };
    }
